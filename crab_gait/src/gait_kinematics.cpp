...
bool GaitKinematics::init() {
	...
	node.param("duration_tripod", d_tripod, 1.0);
        node.param("duration_single", d_single, 1.0);                                   
        node.param("duration_backlegbroken", d_backlegbroken, 1.0);                     
        node.param("duration_middlelegbroken", d_middlelegbroken, 1.0);  
        node.param("duration_fourlegmovement", d_fourlegmovement, 1.0);                                    
...
void GaitKinematics::gaitGenerator(){
	...
	while (node.ok()){
		if (gait_command.cmd == gait_command.RUNRIPPLE){
		...
		else if (gait_command.cmd == gait_command.RUNSINGLE){                                           
			final_vector = gait.RunSingle(frames.begin(), gait_command.fi, gait_command.scale,      
	                        gait_command.alpha, d_single);                                                  
		        ROS_FATAL("RUNSINGLE");						 	        	
			if (callService(final_vector)){                                                         
				joints_pub.publish(legs);                                                       
			}                                                                                       
		...
		else if (gait_command.cmd == gait_command.BACKLEGBROKEN){                                       
			
		    	final_vector = gait.BackLegBroken(frames.begin(), gait_command.fi, gait_command.scale,  
		                gait_command.alpha, d_backlegbroken);                                    			
		    	ROS_FATAL("BACKLEGBROKEN");
			...
			if (callService(final_vector)){                                                         
		    	...
		    	final_vector = gait.MiddleLegBroken(frames.begin(), gait_command.fi, gait_command.scale,  
		                gait_command.alpha, d_middlelegbroken);                                    	  
		    	ROS_FATAL("MIDDLELEGBROKEN");
			...
			if (callService(final_vector)){                                                         
		    	...
		    	final_vector = gait.FourLegMovement(frames.begin(), gait_command.fi, gait_command.scale,  
		                gait_command.alpha, d_fourlegmovement);                                    	  
		    	ROS_FATAL("FOURLEGMOVEMENT");
