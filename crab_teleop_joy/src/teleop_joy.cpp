...
void TeleopJoy::joyCallback(const sensor_msgs::Joy::ConstPtr& joy){
	...
	if (joy->buttons[button_triangle_switch]){
		if (!triangle_flag){
			triangle_flag = true;
	...
	if (joy->buttons[button_circle_switch]){                          
		if (!circle_flag){                                        
		    circle_flag = true;                                   
		    gait_command.cmd = gait_command.STOP;              
		}                                                      
	 ...
	 if (joy->buttons[button_square_switch]){                          
		if (!square_flag){                                        
		    square_flag = true;                                   
		    gait_command.cmd = gait_command.STOP;              
		}                                                      
	...
	if (start_flag){
		...
		if (!joy->buttons[button_left_shift] && !joy->buttons[button_right_shift]){
			if (joy->axes[axis_fi_x] != 0 || joy->axes[axis_fi_y] != 0){
		        	//When the circle is pressed, the gait will be the LEGMOVEMENT
				ROS_INFO_STREAM("  triangle_flag  " << triangle_flag << "  circle_flag  " << circle_flag <<  "  square_flag  " << square_flag);
				if (triangle_flag == true && circle_flag == false){                        
				    gait_command.cmd = gait_command.RUNRIPPLE;                  
				}                                                               
				else if (triangle_flag == true && circle_flag == true){                   
				    gait_command.cmd = gait_command.RUNTRIPOD;                  
				}                                                               
				else if (triangle_flag == false && circle_flag == false && square_flag == false){                   
				    gait_command.cmd = gait_command.RUNSINGLE;                  
				}   
				else if (triangle_flag == false && circle_flag == false && square_flag == true){                  
				    gait_command.cmd = gait_command.FOURLEGMOVEMENT;              
				}                                                              
				else if (triangle_flag == false && circle_flag == true && square_flag == false){                  
				    gait_command.cmd = gait_command.BACKLEGBROKEN;              
				}   
				else if (triangle_flag == false && circle_flag == true && square_flag == true){                  
				    gait_command.cmd = gait_command.MIDDLELEGBROKEN;              
				}   
