...
bool BodyKinematics::init() {
	...
	if (calculateKinematics(&bs)){
		        ROS_FATAL("calculateKInematics_1");						 	        //Anadido por Javier Fernandez
...
void BodyKinematics::teleopBodyMove(const crab_msgs::BodyStateConstPtr &body_state){
	...
	if (calculateKinematics(&bs)){
		ROS_FATAL("calculateKInematics_2");						 	        //Anadido por Javier Fernandez
		joints_pub.publish(legs);
	}
}
...
//		ROS_ERROR("STAND_UP_CMD");
		...
		while (bs.z >= -z){
		  ...
		  r.sleep();
		  if (calculateKinematics(&bs)){
		        	ROS_FATAL("STAND_UP_CMD");						 	        //Anadido por Javier Fernandez
				joints_pub.publish(legs);
		  }
		}
	}
...
//		ROS_ERROR("SEAT_DOWN_CMD");
		...
		while (bs.z <= -0.016){
		  ...
		  r.sleep();
		  if (calculateKinematics(&bs)){
		        	ROS_FATAL("SEAT_DOWN_CMD");						 	        //Anadido por Javier Fernandez
				joints_pub.publish(legs);
		  }
		}
	}
